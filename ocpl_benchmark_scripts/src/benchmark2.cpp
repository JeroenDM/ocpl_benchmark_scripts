...
int main(int argc, char** argv)
{
    ...
    for (auto s : readLinesFromFile("benchmark2/sample_settings.txt"))
    {
        if (s != "")
        {
    ...
    for (auto s : readLinesFromFile("benchmark2/grid_settings.txt"))
    {
        if (s != "")
        {
    ...
    for (auto setting : base_settings)
    {
        ...
        else if (setting.sampler_type == SamplerType::GRID)
        {
            for (std::size_t i{ 0 }; i < grid_sizes.size(); ++i)
            {
                ...
                new_setting.tsr_resolution = { 1, 1, 1, grid_sizes[i].at(1), grid_sizes[i].at(2), grid_sizes[i].at(3) };
                settings.emplace_back(new_setting);
            }
        }
    }
